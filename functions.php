...
function get_marctv_last_commented_articles() {
    ...
    if (!$html) {
        $html = '<div class="container docked"><div class="supertitle"><span><a rel="nofollow" href="http://feeds.feedburner.com/marctv/comments">Zuletzt kommentiert</a></span></div></div>';
        $html.= '<ul class="container multi nohover showontouch">';
        ...
        foreach ($results as $result) {
            ...
            foreach ($comments as $comment) {
                ...
                $comment_user = '<div class="comment-teaser">' . get_avatar($comment->comment_author_email, $size = '32') . '<div class="fn">von ' . $comment->comment_author . ' </div></div>';
            }
...
?>
  ...
  <?php
    ...
    <?php
    printf(__('<cite class="fn">%s</cite> <span class="says">sagt:</span>'), get_comment_author_link()) ?>
      </div>
    <?php
    ...
    do_action('flag_comment_link'); ?> <a href="<?php
    echo htmlspecialchars(get_comment_link($comment->comment_ID)) ?>"><span><?php
    printf(__('%1$s'), get_comment_date('j. M Y')); ?></span></a><?php
    edit_comment_link(__('(Edit)'), '  ', ''); ?></div>
    <?php
    comment_text(); ?>
      <div class="reply">
    <?php
    comment_reply_link(array_merge($args, array('reply_text' => 'Antworten', 'depth' => $depth, 'max_depth' => $args['max_depth']))) ?>
      </div>
    </div>
    <?php
}
